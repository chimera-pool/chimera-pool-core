...
func (suite *E2ETestSuite) TestSecurityAndErrorHandling() {
	...
	// Test 3: Service error handling - pool stats endpoint returns success by default
	// This test verifies the error response format when service errors occur
	// Note: Mock may already have expectations set, so we verify the response format
	req = httptest.NewRequest("GET", "/api/v1/pool/stats", nil)
	w = httptest.NewRecorder()
	suite.router.ServeHTTP(w, req)

	// Pool stats is a public endpoint, should return 200 with valid mock
