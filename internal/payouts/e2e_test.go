...
func TestE2E_CompleteMiningToPayoutWorkflow(t *testing.T) {
	...
	for _, payout := range db.payouts {
		...
		assert.WithinDuration(t, block.Timestamp, payout.Timestamp, time.Second, "Payout timestamp should match block time")
...
func TestE2E_MultipleBlocksScenario(t *testing.T) {
	...
	block1 := Block{
		...
		Reward: 2000000000, Status: "confirmed", Timestamp: now.Add(-20 * time.Minute),
	}
	...
	block2 := Block{
		...
		Reward: 2500000000, Status: "confirmed", Timestamp: now.Add(-5 * time.Minute),
	}
